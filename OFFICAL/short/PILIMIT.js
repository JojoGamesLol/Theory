...
var tick = (elapsedTime, multiplier) => {
    ...
    let bSUM = getB1(b1.level) * getB2(b2.level) - BigNumber.from(q).pow(BigNumber.from(qExp));
...
var getTertiaryEquation = () => {
    ...
    result += qExp;
    result += "} = ";
    result += BigNumber.from(q).pow(BigNumber.from(qExp));
...
const getA1 = (level) => 1 + BigNumber.from(level);
const getA2 = (level) => 1 + BigNumber.from(level);
